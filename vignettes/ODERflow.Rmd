...
annot_ers <- annotatERs(
    ...
    gtf_path = gtf_path, chrs_to_keep = c("21"), ensembl = TRUE, genom_state = genomic_state
)
...
ann_opt_ers <- get_junctions(opt_ers = head(opt_ers[["opt_ers"]], 100), junc_data = junctions, gtf_path = gtf_path)
```
...
annot_ers <- ann_opt_ers[S4Vectors::mcols(ann_opt_ers)[["annotation"]] %in% c("intron", "intergenic")]
